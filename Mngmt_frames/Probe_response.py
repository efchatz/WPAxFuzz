...
class Proberesp(Frame):
    ...
    def send_probe_resp_with_rand_SSID(self, mode):
        ...
        frame = self.MAC_header(mode) / probe_resp / self.generate_SSID(self.generator, mode) / SUPPORTED_RATES / SUPPL_RATES / STANDARD_DS / STANDARD_RM_CAPS / STANDARD_HT_CAPABILITIES / STANDARD_HT_INFORMATION / STANDARD_EXT_HT_CAPABILITIES / STANDARD_RSN
        return frame
    ...
    def send_probe_resp_with_rand_source_mac(self, mode):
        probe_resp = Dot11ProbeResp(cap=4920)
        frame = self.construct_MAC_header(5, self.dest_addr, self.generate_MAC(self.generator), self.source_addr) / probe_resp / self.ssid / SUPPORTED_RATES / SUPPL_RATES / STANDARD_DS / STANDARD_RM_CAPS / STANDARD_HT_CAPABILITIES / STANDARD_HT_INFORMATION / STANDARD_EXT_HT_CAPABILITIES / STANDARD_RSN
        return frame

    def send_probe_resp_with_rand_supp_speed(self, mode):
        probe_resp = Dot11ProbeResp(cap=4920)
        frame = self.MAC_header(mode) / probe_resp / self.ssid / self.generate_supp_speed(self.generator, mode) / STANDARD_DS / STANDARD_RM_CAPS / STANDARD_HT_CAPABILITIES /STANDARD_HT_INFORMATION / STANDARD_EXT_HT_CAPABILITIES / STANDARD_RSN
        return frame

    def send_probe_resp_with_rand_DSset(self, mode):
        probe_resp = Dot11ProbeResp(cap=4920)
        frame = self.MAC_header(mode) / probe_resp / self.ssid / SUPPORTED_RATES / SUPPL_RATES / self.generate_channel_use(self.generator, mode) / STANDARD_RM_CAPS / STANDARD_HT_CAPABILITIES / STANDARD_HT_INFORMATION / STANDARD_EXT_HT_CAPABILITIES / STANDARD_RSN
        return frame

    def send_probe_resp_with_rand_HT_capabilities(self, mode):
        probe_resp = Dot11ProbeResp(cap=4920)
        frame = self.MAC_header(mode) / probe_resp / self.ssid / SUPPORTED_RATES / SUPPL_RATES / STANDARD_DS / STANDARD_RM_CAPS / self.generate_HT_capabilities(self.generator, mode) / STANDARD_HT_INFORMATION / STANDARD_EXT_HT_CAPABILITIES / STANDARD_RSN
        return frame

    def send_probe_resp_with_rand_HT_information(self, mode):
        probe_resp = Dot11ProbeResp(cap=4920)
        frame = self.MAC_header(mode) / probe_resp / self.ssid / SUPPORTED_RATES / SUPPL_RATES / STANDARD_DS / STANDARD_RM_CAPS / STANDARD_HT_CAPABILITIES / self.generate_HT_information(self.generator, mode) / STANDARD_EXT_HT_CAPABILITIES / STANDARD_RSN
        return frame

    def send_probe_resp_with_rand_RM_enabled_capabilities(self, mode):
        probe_resp = Dot11ProbeResp(cap=4920)
        frame = self.MAC_header(mode) / probe_resp / self.ssid / SUPPORTED_RATES / SUPPL_RATES / STANDARD_DS / self.generate_RM_enabled_capabilities(self.generator, mode) / STANDARD_HT_CAPABILITIES / STANDARD_HT_INFORMATION / STANDARD_EXT_HT_CAPABILITIES / STANDARD_RSN
        return frame

    def send_probe_resp_with_rand_extended_HT_capabilities(self, mode):
        probe_resp = Dot11ProbeResp(cap=4920)
        frame = self.MAC_header(mode) / probe_resp / self.ssid / SUPPORTED_RATES / SUPPL_RATES / STANDARD_DS / STANDARD_RM_CAPS / STANDARD_EXT_HT_CAPABILITIES / STANDARD_HT_INFORMATION / self.generate_extended_HT_capabilities(self.generator, mode) / STANDARD_RSN
        return frame
