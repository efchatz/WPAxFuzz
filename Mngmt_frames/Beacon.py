...
class Beacon(Frame):
    ...
    def send_beacon_with_rand_SSID(self, mode):
        ...
        frame = self.MAC_header(mode) / beacon / self.generate_SSID(self.fuzzer, mode) / SUPPORTED_RATES / SUPPL_RATES / STANDARD_DS / STANDARD_HT_CAPABILITIES / STANDARD_HT_INFORMATION / STANDARD_EXT_HT_CAPABILITIES / STANDARD_TIM / STANDARD_RM_CAPS / STANDARD_RSN
        return frame
    ...
    def send_beacon_with_rand_source_mac(self, mode):
        beacon = Dot11Beacon(cap='ESS+privacy')
        frame = self.construct_MAC_header(8, 'ff:ff:ff:ff:ff:ff', self.generate_MAC(self.fuzzer), self.source_addr) / beacon / self.ssid / SUPPORTED_RATES / SUPPL_RATES / STANDARD_DS / STANDARD_HT_CAPABILITIES / STANDARD_HT_INFORMATION / STANDARD_EXT_HT_CAPABILITIES / STANDARD_TIM / STANDARD_RM_CAPS / STANDARD_RSN
        return frame

    def send_beacon_with_rand_TIM(self, mode):
        beacon = Dot11Beacon(cap='ESS+privacy')
        frame = self.MAC_header(mode) / beacon / self.ssid / SUPPORTED_RATES / SUPPL_RATES / STANDARD_DS / STANDARD_HT_CAPABILITIES / STANDARD_HT_INFORMATION / STANDARD_EXT_HT_CAPABILITIES / self.construct_TIM(self.fuzzer,mode) / STANDARD_RM_CAPS / STANDARD_RSN
        return frame

    def send_beacon_with_rand_supp_speed(self, mode):
        beacon = Dot11Beacon(cap='ESS+privacy')
        frame = self.MAC_header(mode) / beacon / self.ssid / self.generate_supp_speed(self.fuzzer, mode) / STANDARD_DS / STANDARD_HT_CAPABILITIES / STANDARD_HT_INFORMATION/ STANDARD_EXT_HT_CAPABILITIES / STANDARD_TIM / STANDARD_RM_CAPS / STANDARD_RSN
        return frame

    def send_beacon_with_rand_DSset(self, mode):
        beacon = Dot11Beacon(cap='ESS+privacy')
        frame = self.MAC_header(mode) / beacon / self.ssid / SUPPORTED_RATES / SUPPL_RATES / self.generate_channel_use(self.fuzzer, mode) / STANDARD_HT_CAPABILITIES / STANDARD_HT_INFORMATION / STANDARD_EXT_HT_CAPABILITIES/\
                STANDARD_TIM / STANDARD_RM_CAPS / STANDARD_RSN
        return frame

    def send_beacon_with_rand_HT_capabilities(self, mode):
        beacon = Dot11Beacon(cap='ESS+privacy')
        frame = self.MAC_header(mode) / beacon / self.ssid / SUPPORTED_RATES / SUPPL_RATES / STANDARD_DS / self.generate_HT_capabilities(self.fuzzer, mode) / STANDARD_HT_INFORMATION / STANDARD_EXT_HT_CAPABILITIES / STANDARD_TIM / STANDARD_RM_CAPS / STANDARD_RSN
        return frame

    def send_beacon_with_rand_RM_enabled_capabilities(self, mode):
        beacon = Dot11Beacon(cap='ESS+privacy')
        frame = self.MAC_header(mode) / beacon / self.ssid / SUPPORTED_RATES / SUPPL_RATES / STANDARD_DS / STANDARD_HT_CAPABILITIES / STANDARD_HT_INFORMATION / STANDARD_EXT_HT_CAPABILITIES / STANDARD_TIM / self.generate_RM_enabled_capabilities(self.fuzzer, mode) / STANDARD_RSN
        return frame
    ...
    def send_beacon_with_rand_extended_HT_capabilities(self, mode):
        beacon = Dot11Beacon(cap='ESS+privacy')
        frame = self.MAC_header(mode) / beacon / self.ssid / SUPPORTED_RATES / SUPPL_RATES / STANDARD_DS / STANDARD_HT_CAPABILITIES / STANDARD_HT_INFORMATION / self.generate_extended_HT_capabilities(self.fuzzer, mode) / STANDARD_TIM / STANDARD_RM_CAPS / STANDARD_RSN
        return frame
