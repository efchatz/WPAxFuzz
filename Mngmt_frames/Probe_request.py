...
class ProbeReq(Frame):
    ...
    def send_Probe_req_with_rand_RSN(self, mode):
        ...
        frame = self.construct_MAC_header(4, self.dest_addr, self.source_addr, self.dest_addr) / probe_req / \
                self.ssid / SUPPORTED_RATES / SUPPL_RATES / STANDARD_DS / STANDARD_HT_CAPABILITIES / STANDARD_EXT_HT_CAPABILITIES / self.construct_RSN(self.generator, mode)
        return frame

    def send_Probe_req_with_rand_source_mac(self, mode):
        probe_req = Dot11ProbeReq()
        frame = self.construct_MAC_header(4, self.dest_addr, self.generate_MAC(self.generator), self.dest_addr) / probe_req / \
    ...
    def send_Probe_req_with_rand_supp_speed(self, mode):
        probe_req = Dot11ProbeReq()
        frame = self.construct_MAC_header(4, self.dest_addr, self.source_addr, self.dest_addr) / probe_req / \
                self.ssid / self.generate_supp_speed(self.generator, mode) / STANDARD_DS / STANDARD_HT_CAPABILITIES / STANDARD_EXT_HT_CAPABILITIES / STANDARD_RSN
        return frame

    def send_Probe_req_with_rand_DSset(self, mode):
        probe_req = Dot11ProbeReq()
        frame = self.construct_MAC_header(4, self.dest_addr, self.source_addr, self.dest_addr) / probe_req / \
                self.ssid / SUPPORTED_RATES / SUPPL_RATES / self.generate_channel_use(self.generator, mode) / STANDARD_HT_CAPABILITIES / STANDARD_EXT_HT_CAPABILITIES / STANDARD_RSN
        return frame

    def send_Probe_req_with_rand_HT_capabilities(self, mode):
        probe_req = Dot11ProbeReq()
        frame = self.construct_MAC_header(4, self.dest_addr, self.source_addr, self.dest_addr) / probe_req / \
                self.ssid / SUPPORTED_RATES / SUPPL_RATES / STANDARD_DS / self.generate_HT_capabilities(self.generator, mode) / STANDARD_EXT_HT_CAPABILITIES / STANDARD_RSN
        return frame

    def send_Probe_req_with_rand_ext_HT_capabilities(self, mode):
        probe_req = Dot11ProbeReq()
        frame = self.construct_MAC_header(4, self.dest_addr, self.source_addr, self.dest_addr) / probe_req / \
                self.ssid / SUPPORTED_RATES / SUPPL_RATES / STANDARD_DS / STANDARD_HT_CAPABILITIES / self.generate_extended_HT_capabilities(self.generator, mode) / STANDARD_RSN
        return frame
