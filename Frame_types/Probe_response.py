...
class Proberesp(Frame):
    ...
    def __init__(self, mode, frame_name, dest_addr, source_addr, interface, ssid):
        ...
        self.fuzzer_state = {
            "empty": {
                "send_function": self.send_empty_probe_resp,
                "conn_loss": False
            },
            ...
            "addresses reversed '(destination = AP, source = STA)'": {
                "send_function": self.send_probe_resp_with_reverse_addresses,
                "conn_loss": False
            },
            "all fields": {
    ...
    def send_empty_probe_resp(self, mode):
        return self.construct_MAC_header(5, self.dest_addr, self.source_addr, self.source_addr)
    ...
    def send_probe_resp_with_rand_timestamp_interval(self, mode):
        ...
        frame = self.construct_MAC_header(5, self.dest_addr, self.source_addr, self.source_addr) / probe_resp / \
    ...
    def send_probe_resp_with_rand_SSID(self, mode):
        ...
        frame = self.construct_MAC_header(5, self.dest_addr, self.source_addr, self.source_addr) / probe_resp / \
                self.generate_SSID(mode) / SUPPORTED_RATES / SUPPL_RATES / STANDARD_DS / STANDARD_RM_CAPS / STANDARD_HT_CAPABILITIES / \
                STANDARD_HT_INFORMATION / STANDARD_EXT_HT_CAPABILITIES / STANDARD_RSN
        return frame
    ...
    def send_probe_resp_with_rand_RSN(self, mode):
        ...
        frame = self.construct_MAC_header(5, self.dest_addr, self.source_addr, self.source_addr) / probe_resp / \
                self.ssid / SUPPORTED_RATES / SUPPL_RATES / STANDARD_DS / STANDARD_RM_CAPS / STANDARD_HT_CAPABILITIES / \
                STANDARD_HT_INFORMATION / STANDARD_EXT_HT_CAPABILITIES / self.construct_RSN(mode)
        return frame

    def send_probe_resp_with_rand_source_mac(self, mode):
        probe_resp = Dot11ProbeResp(cap=4920)
        frame = self.construct_MAC_header(5, self.dest_addr, self.generate_MAC(), self.source_addr) / probe_resp / \
                self.ssid / SUPPORTED_RATES / SUPPL_RATES / STANDARD_DS / STANDARD_RM_CAPS / STANDARD_HT_CAPABILITIES / \
                STANDARD_HT_INFORMATION / STANDARD_EXT_HT_CAPABILITIES / STANDARD_RSN
        return frame
    ...
    def send_probe_resp_with_rand_supp_speed(self, mode):
        probe_resp = Dot11ProbeResp(cap=4920)
        frame = self.construct_MAC_header(5, self.dest_addr, self.source_addr, self.source_addr) / probe_resp / \
                self.ssid / self.generate_supp_speed(mode) / STANDARD_DS / STANDARD_RM_CAPS / STANDARD_HT_CAPABILITIES / \
                STANDARD_HT_INFORMATION / STANDARD_EXT_HT_CAPABILITIES / STANDARD_RSN
        return frame
    ...
    def send_probe_resp_with_rand_DSset(self, mode):
        probe_resp = Dot11ProbeResp(cap=4920)
        frame = self.construct_MAC_header(5, self.dest_addr, self.source_addr, self.source_addr) / probe_resp / \
                self.ssid / SUPPORTED_RATES / SUPPL_RATES / self.generate_channel_use(mode) / STANDARD_RM_CAPS / STANDARD_HT_CAPABILITIES / \
                STANDARD_HT_INFORMATION / STANDARD_EXT_HT_CAPABILITIES / STANDARD_RSN
        return frame
    ...
    def send_probe_resp_with_rand_HT_capabilities(self, mode):
        probe_resp = Dot11ProbeResp(cap=4920)
        frame = self.construct_MAC_header(5, self.dest_addr, self.source_addr, self.source_addr) / probe_resp / \
                self.ssid / SUPPORTED_RATES / SUPPL_RATES / STANDARD_DS / STANDARD_RM_CAPS / self.generate_HT_capabilities(mode) / \
                STANDARD_HT_INFORMATION / STANDARD_EXT_HT_CAPABILITIES / STANDARD_RSN
        return frame
    ...
    def send_probe_resp_with_rand_HT_information(self, mode):
        ...
        frame = self.construct_MAC_header(5, self.dest_addr, self.source_addr, self.source_addr) / probe_resp / \
                self.ssid / SUPPORTED_RATES / SUPPL_RATES / STANDARD_DS / STANDARD_RM_CAPS / STANDARD_HT_CAPABILITIES / \
                self.generate_HT_information(mode) / STANDARD_EXT_HT_CAPABILITIES / STANDARD_RSN
        return frame

    def send_probe_resp_with_rand_RM_enabled_capabilities(self, mode):
        probe_resp = Dot11ProbeResp(cap=4920)
        frame = self.construct_MAC_header(5, self.dest_addr, self.source_addr, self.source_addr) / probe_resp / \
                self.ssid / SUPPORTED_RATES / SUPPL_RATES / STANDARD_DS / self.generate_RM_enabled_capabilities(mode) / \
                STANDARD_HT_CAPABILITIES / STANDARD_HT_INFORMATION / STANDARD_EXT_HT_CAPABILITIES / STANDARD_RSN
        return frame

    def send_probe_resp_with_rand_extended_HT_capabilities(self, mode):
        probe_resp = Dot11ProbeResp(cap=4920)
        frame = self.construct_MAC_header(5, self.dest_addr, self.source_addr, self.source_addr) / probe_resp / \
                self.ssid / SUPPORTED_RATES / SUPPL_RATES / STANDARD_DS / STANDARD_RM_CAPS / STANDARD_EXT_HT_CAPABILITIES / \
                STANDARD_HT_INFORMATION / self.generate_extended_HT_capabilities(mode) / STANDARD_RSN
        return frame
