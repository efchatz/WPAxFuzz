...
class Frame:
    ...
    def generate_bytes(self, num_of_bytes, mode):
...
A-F]' -n {num_of_bytes} '''], stdout=subprocess.PIPE, shell=True)
    ...
    # 00 and 14-15 are reserved for AKMS
        ...
        for item in self.generate_bytes(1, mode):
            rsn_array[1].append(item)
        for item in self.generate_bytes(1, mode):
            rsn_array[3].append(item)
        for item in self.generate_bytes(1, mode):
            rsn_array[5].append(item)
        for item in self.generate_bytes(2, mode):
    ...
    def construct_TIM(self, mode):
        ...
        for item in self.generate_bytes(6, mode):
            tim_bytes.append(item)
        return Dot11Elt(ID='TIM', info=tim_bytes)

    def generate_MAC(self):
        mac_bytes = self.generate_bytes(6, 'standard')
    ...
    def generate_SSID(self, mode):
        ssid = bytearray(b'')
        for item in self.generate_bytes(16, mode):
            ssid.append(item)
    ...
    def generate_supp_speed(self, mode):
        ...
        for item in self.generate_bytes(8, mode):
            supported_rates.append(item)
        for item in self.generate_bytes(4, mode):
    ...
    def generate_HT_capabilities(self, mode):
        ht_cap = bytearray(b'')
        for item in self.generate_bytes(26, mode):
            ht_cap.append(item)
